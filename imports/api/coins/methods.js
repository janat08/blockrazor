...
import {
    rewardCurrencyCreator
} from '/imports/api/utilities.js';
import {
    quality
} from '/imports/api/utilities'
import SimpleSchema from 'simpl-schema';

//check for contidiontal checks if or not proposal or btcfork and etc.

export const addCoin = new ValidatedMethod({
    name: 'addCoin',
    validate: 
    // new SimpleSchema({currencyName: String}),
    new SimpleSchema(Currencies.schema.pick('currencyName', 'currencySymbol', 'premine', 'maxCoins', 'consensusSecurity', 'gitRepo', 
    'officialSite', 'reddit', 'blockExplorer', 'approvalNotes', 'currencyLogoFilename', 'confirmations', 'previousNames', 'exchanges', 
    'launchTags', 'blcokTime', 'forkHeight', 'forkParent', 'hashAlgorithm', 'ICOfundsRaised', 'genesisTimestamp', 'proposal', 'altcoin', 
    'ico', 'ICOcoinsProduced', 'ICOcoinsIntended',  'ICOnextRound', 'icoDateEnd', 'btcfork'), 
    {requiredByDefault: developmentValidationEnabledFalse }).validator(),
    // .extend({
    //     currencyName: { type: String, autoValue: ()=>uniqueness check or something}
    // }),
    // schemaValidatorOptions: { clean: {getAutoValues: true}, filter: false, requiredBydefault: developmentValidationEnabledFalse },
    ...
    Meteor.methods({
        ...
        reviewCurrency: function (currencyId) {
            if (UserData.findOne({
                ...
                var original = PendingCurrencies.findOne({
                    _id: currencyId
                });
                return original;
            }
            ...
            if (UserData.findOne({
                ...
                Currencies.insert(insert, function (error, result) {
                    if (!error) {
                        ...
                        if (rewardCurrencyCreator(original.launchTags, original.owner, original.currencyName)) {
        ...
        isCurrencyNameUnique(name) {
            ...
            if (name) {
                ...
                var res = PendingCurrencies.find({}, {
                    fields: {
                        currencyName: 1,
                        id: -1
                    }
                }).fetch().concat(Currencies.find({}, {
                    fields: {
                        currencyName: 1,
                        id: -1
                    }
